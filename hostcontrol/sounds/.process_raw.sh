...
for in_file in "${files[@]}"; do
  ...
  if ffmpeg -hide_banner -loglevel error -y \
      -i "$in_file" \
      -af "loudnorm=I=-16:TP=-1.5:LRA=11,silenceremove=start_periods=1:start_silence=0:start_threshold=-60dB,areverse,silenceremove=start_periods=1:start_silence=0:start_threshold=-60dB,areverse" \
