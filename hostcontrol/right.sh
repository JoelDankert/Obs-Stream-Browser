...
# default: play the random wildcard sound if present
random_sounds=()
while read -r f; do
  random_sounds+=("$f")
done < <(find "$SOUND_DIR" -maxdepth 1 -type f -name 'random*.mp3' -printf '%f\n' | sort)
[ ${#random_sounds[@]} -eq 0 ] && exit 0
sound_file=${random_sounds[RANDOM % ${#random_sounds[@]}]}
ffplay -nodisp -loglevel quiet -autoexit -af "volume=$VOLUME" "$SOUND_DIR/$sound_file" </dev/null >/dev/null 2>&1 &
