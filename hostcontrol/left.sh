...
boom_sounds=(boom0.mp3 boom1.mp3 boom2.mp3 boom3.mp3)
boom_sounds=()
while read -r f; do
  boom_sounds+=("$f")
done < <(find "$SOUND_DIR" -maxdepth 1 -type f -name 'boom*.mp3' -printf '%f\n' | sort)
[ ${#boom_sounds[@]} -eq 0 ] && exit 0
pick=${boom_sounds[RANDOM % ${#boom_sounds[@]}]}
